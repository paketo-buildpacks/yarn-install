...
func testClassicInstallProcess(t *testing.T, context spec.G, it spec.S) {
	...
	context("ShouldRun", func() {
		var (
			...
			installProcess classic.YarnInstallProcess
			summer         *fakes.Summer
		...
		it.Before(func() {
			...
			installProcess = classic.NewYarnInstallProcess(executable, summer, scribe.NewEmitter(bytes.NewBuffer(nil)))
		})
	...
	context("SetupModules", func() {
		...
		it.Before(func() {
			...
			installProcess = classic.NewYarnInstallProcess(executable, summer, scribe.NewEmitter(buffer))
		})
